...
const AppointmentModal: React.FC<AppointmentModalProps> = ({ isOpen, onClose }) => {
  ...
  const sendWhatsAppMessage = () => {
    ...
    const whatsappNumber = '919599106813';
          ...
          >
            ...
            {state.succeeded ? (
              ...
              <form onSubmit={handleFinalSubmit} className="space-y-4 font-work-sans">
                <div>
                  <label htmlFor="patient_name" className="block text-sm font-medium text-blue-700">Patient's Name</label>
                  <input type="text" id="patient_name" name="patient_name" value={formData.patient_name} onChange={handleChange} required className="mt-1 block w-full border border-blue-500 rounded-md shadow-sm p-2 bg-white text-zinc-900 focus:ring-2 focus:ring-blue-400 focus:border-transparent" />
                </div>
                <div>
                ...
                <div className="flex justify-end space-x-2">
                  <button type="button" onClick={onClose} className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-100 transition-colors duration-300">
                    Cancel
                  </button>
                  <button type="submit" className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-gray-900 bg-white  transition-colors duration-300">
                    Book Appointment
                  </button>
                </div>
              </form>
